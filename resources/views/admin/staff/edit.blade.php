...
<div class="mx-auto max-w-4xl space-y-6">
    <div class="flex items-center justify-between gap-4">
        <div>
            <h2 class="text-xl font-bold text-slate-900">{{ $staff->full_name }}</h2>
            <p class="text-sm text-slate-500">Update this staff member’s account information.</p>
        </div>
        <a href="{{ route('admin.staff') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-50">Back to Staff</a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.staff.update', $staff) }}" class="grid gap-5 rounded-xl border border-slate-200 bg-white p-6 shadow-sm md:grid-cols-2">
        @csrf
        @method('PUT')
        <div>
        ...
        <div class="md:col-span-2 flex justify-end">
            <button type="submit" class="rounded-lg bg-brand-600 px-6 py-2.5 font-semibold text-white transition hover:bg-brand-700">Save Changes</button>
        </div>
    </form>
</div>
@endsection
